...
GETUSER_API_URL = "https://tarz0uu2n5.execute-api.us-east-2.amazonaws.com/prod/getuser-mcp"
GETRECIPIENT_API_URL = "https://tarz0uu2n5.execute-api.us-east-2.amazonaws.com/prod/getrecipient-mcp"
LAMBDA_API_KEY_ONBOARDING = "0h5c9wlW2L8Joqk6fpJUz6NnsIjLB6Su7V62ozcd"


class OnboardingCompleteRequest(BaseModel):
    user_id: str
    assessment_score: Optional[int] = None
    assessment_answers: Optional[list] = None
    tasks: Optional[list] = None


async def _fetch_and_ingest_onboarding(
    user_id: str,
    assessment_score: Optional[int] = None,
    assessment_answers: Optional[list] = None,
    tasks: Optional[list] = None,
) -> dict:
    """Fetch user/recipient data from Lambda APIs and send to WithCare for fact ingestion."""
    headers = {"x-api-key": LAMBDA_API_KEY_ONBOARDING, "Content-Type": "application/json"}

    # 1. Fetch user info
    try:
        resp = requests.post(GETUSER_API_URL, headers=headers, json={"user_Id": user_id}, timeout=10)
        user_info = resp.json()
    except Exception as e:
        logger.error(f"Failed to fetch user info for {user_id}: {e}")
        return {"error": f"Failed to fetch user info: {e}"}

    # 2. Extract care recipients and fetch their data
    care_recipients = []
    for cr in user_info.get("careRecipients", []):
        recipient_id = cr.get("recipient_Id", "")
        relationship = cr.get("relationship", "")
        if not recipient_id:
            continue
        try:
            resp2 = requests.post(GETRECIPIENT_API_URL, headers=headers, json={"recipient_Id": recipient_id}, timeout=10)
            recipient_data = resp2.json()
            care_recipients.append({
                "relationship": relationship,
                "data": recipient_data,
            })
        except Exception as e:
            logger.warning(f"Failed to fetch recipient {recipient_id}: {e}")

    # 3. Build user_data (exclude careRecipients and user_Id)
    user_data = {k: v for k, v in user_info.items() if k not in ("careRecipients", "user_Id")}

    # 4. Send to WithCare /onboarding/ingest
    if not WITHCARE_AGENT_URL:
        logger.warning("WITHCARE_AGENT_URL not configured, skipping onboarding ingest")
        return {"error": "WITHCARE_AGENT_URL not configured"}
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.post(f"{WITHCARE_AGENT_URL}/onboarding/ingest", json=ingest_payload)
            resp.raise_for_status()
            result = resp.json()
            logger.info(f"Onboarding ingest successful for {user_id}: {result.get('total_facts_written', 0)} facts")
            return result
    except Exception as e:
        logger.error(f"Onboarding ingest failed for {user_id}: {e}")
        return {"error": str(e)}


@app.post("/onboarding/complete")
async def onboarding_complete(req: OnboardingCompleteRequest):
    """Called when onboarding finishes. Fetches user data from Lambda APIs
    and sends it to WithCare agent for structured fact ingestion."""
    result = await _fetch_and_ingest_onboarding(
        user_id=req.user_id,
